...
function Ravi_Decrypt(string)
	...
	for i = 1, strlen do
		...
		nChar = String.Mid(string, i, 1);
				
		for j = 1, 26 do	
			if nChar == tbl_RAVI[j] then				
				posInRavi = j;
				break;		
			end	
		end
		...
		diffOfPos = posInRavi - posInString
		
		if IsUpper(nChar) then										--UpperCase Chars
			decText = decText..String.Upper(tbl_ALPHA[-1 * diffOfPos]);
		elseif IsLower(nChar) then									--LowerCase Chars
			decText = decText..tbl_ALPHA[diffOfPos];				
		elseif nChar == " " then									--Space
			decText = decText.." "
		elseif posInRavi == 0 then									--All Other Chars
