...
function Ravi_Encrypt(string)
	...
	for l = 1, strlen do
		...
		nChar = String.Mid(string, l, 1);	--Get the string character by character
...
Debug.Print("NewDiffOfPos = "..diffOfPos.."\r\n");
		...
		if nChar == " " then
			decText = decText.." ";
		elseif posInRavi == 0 and posInRaviCaps == 0 and IsNumber(nChar) == false then		--Non-Alphanumeric
