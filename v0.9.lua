...
Debug.Print("NewSumOfPos = "..sumOfPos.."\r\n");
		...
		if nChar == " " then								--Space
			encText = encText.." ";
		elseif posInAlpha == 0 and posInAlphaCaps == 0 then	--Non-Alphabets.
		...
		elseif IsNumber(nChar) and encNum == true then		--Numbers	
...
if Task == "Encrypt" then
	...
	if CheckBox.GetChecked("encNum") then
		local dlgRes = Dialog.Message("Be Carefull", "You won\'t be able to decrypt the text successfully once the numbers are encrypted.\\n\\nDo you want to continue?", MB_YESNO, MB_ICONQUESTION, MB_DEFBUTTON1)
